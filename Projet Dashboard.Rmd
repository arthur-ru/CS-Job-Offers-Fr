...
server <- function(input, output) {
  ...
  salaire_summary <- summary(finalData$salaire)
  
  # Create a data frame to store the summary statistics
  summary_table <- data.frame(
    Statistic = c("Minimum", "1st Quartile", "Median", "Mean", "3rd Quartile", "Maximum"),
    Value = c(min(finalData$salaire), quantile(finalData$salaire, 0.25), median(finalData$salaire), mean(finalData$salaire), quantile(finalData$salaire, 0.75), max(finalData$salaire))
  )
...
### Interpretation
```{r}
p("- The dataset is somewhat right-skewed since the mean (average) is slightly higher than the median.")
p("- Most of the data points (75%) lie within the range of approximately 13,464 to 43,653.")
p("- The maximum value of 90,000 suggests that there are some outliers in the dataset, which are significantly higher than the rest of the values. These outliers could represent unusually high salaries.")
```

### plot
```{r}
plot_ly(finalData, y = ~salaire, type = "box") %>%
