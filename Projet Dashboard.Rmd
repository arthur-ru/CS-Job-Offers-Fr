...
fluidRow(
  column(width = 4, 
         imageOutput("logo", height = "100px", width = "100px"))
)
```
...
leaflet(data = france_departments) %>%
  ...
  addPolygons(
    ...
    opacity = 1,
    smoothFactor = 0.7,
    popup = ~paste("Number of offers in ",nom, ": ", nb_offers)
  )
})
...
```{r}
finalData$skills <- gsub("\\[|\\]|'", "", finalData$skills)
finalData$skills <- strsplit(finalData$skills, ", ")
df_explose <- finalData %>% unnest(cols = skills)
df_skills <- unique(df_explose$skills)
skills_info <- list()
for (skill in df_skills) {
    count <- sum(df_explose$skills == skill)
    skills_info[[skill]] <- c(count)
}
skills_df <- as.data.frame(do.call(rbind, skills_info))
colnames(skills_df) <- c("Nombre d'occurrences")
skills_df_ordered <- arrange(skills_df, desc(`Nombre d'occurrences`))
skills_df_ordered
