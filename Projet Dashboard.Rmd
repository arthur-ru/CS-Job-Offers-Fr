...
server <- function(input, output) {
  ...
  output$pieChart <- renderPlotly({
    ...
    plot_ly(skills_info, labels = ~skills, values = ~`Nombre d'occurrences`, type = 'pie') %>%
      layout(title = "Most asked skills by sector")
  })
}
...
renderPlotly({
  ...
  ggplot(location_data, aes(x = salaire)) +
    ...
    labs(title = "Salary repartition by region",
         x = "Salaries", y = "") +
    theme_minimal() +
    theme(plot.margin = margin(1, 1, 2.5, 1, "cm"))
})
```
...
# Appliquer la fonction à la colonne des salaires
finalData$salaire2 <- arrondir_dizaine_millier(finalData$salaire)
finalData<- finalData[finalData$salaire2 != 40000 | sample(c(TRUE, FALSE), nrow(finalData), replace = TRUE, prob = c(0.1, 0.9)), ]
finalData <- finalData %>%
  mutate(teletravail = ifelse(teletravail == "False", -1, 1))
finalData$nombre_de_skills <- str_count(finalData$skills, "\\w+")
data_selected <- finalData[, c("teletravail", "department", "nombre_de_skills", "salaire2")]
data_selected <- na.omit(data_selected)
corrplot(cor(data_selected), method="ellipse")
